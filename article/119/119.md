...
从玩家的电脑到服务器、其他玩家电脑的网络，中间任何一个环节都需要不出问题，才能顺畅游玩。但这其中大部分网络都不是玩家能直接控制的。且不说运营商的路由策略没有保证，现在连光猫超管密码都不容易获取。
...
**自建代理**
- 需要一定的网络知识才能配置正确。
- 如果达到和加速器一样的效果，由于是零售，一般比加速器还高。虽然自建节点即能翻墙又能游戏，但总的费用仍然不能忽视。
...
这是网友 [Dogfight360](https://www.dogfight360.com/blog/) 在持续维护软件。我简单分析一下（不一定正确）：  
...
Steam 客户端支持系统代理，也支持配置 SOCKS5 代理网页浏览（确定是支持 SOCKS5，不是因为代理客户端的混合监听端口）。代理设置对同一台电脑上的所有帐户都生效。  
但是配置方式比较隐蔽：  
先在右下角托盘图标右键，进入「大屏幕」模式，然后：菜单 → 设置 → 互联网 → HTTP 代理，如图：  

![Steam代理.png](./Steam代理.png)  

写 `socks5://` 是有效的：  

![Steam代理2.png](./Steam代理2.png)  

感谢：[知乎@懒惰可耻](https://zhuanlan.zhihu.com/p/671363488)  

**本段内容于 2024-12 月验证**  

Steam 客户端会自动判断所在区域，并分配最近的下载服务器；可以在 `设置 → 下载 → 下载地区` 确认。或者`运行(Win+R)` `steam://open/console`，打开 Steam 控制台，输入 `user_info` ，回车，查看 IPCountry。  
...
Windows 上的 Xbox 应用、身份验证应用是 **UWP 应用，网络环境默认是隔离的，需要解除回环限制能才访问系统代理**。  
可以用 Fiddler（v2rayN、Clash 等）附带的 **EnableLoopback.exe**（解除 Win10 UWP 应用回环代理限制）。  
...
2023-02 月开始，微软的认证服务器 IP（20.197.103.57）被墙（ [例1](https://www.v2ex.com/t/915450)、[例2](https://ngabbs.com/read.php?tid=30667368) ）。2024-12-13 测试，这个 IP 443 端口 TCP 也还是不通（可能是下线了）。  
...
以下列出**下载域名**，使用的是 Akamai 的服务，「绕过大陆」下会通过代理下载。  
...
如果你使用了代理才出现这个报错，那么 [育碧官方支持](https://www.ubisoft.com/zh-cn/help/connectivity-and-performance/article/error-message-ubisoft-connect-has-detected-an-unrecoverable-error-and-must-shutdown-on-ubisoft-connect-pc/000064974) 的方法是没用的。  
这个问题的原因是**育碧客户端对 HTTP 代理的适配不好**（转载自 [shunf4](https://github.com/2dust/v2rayN/discussions/2158#discussioncomment-9312266) ）：  
...
2026-04-15 更新：Steam 支持配置 SOCKS5 代理，之前我写错了。  
2024-12-14 更新 Xbox 游戏登录设置。  
